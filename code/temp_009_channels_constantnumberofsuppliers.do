...
* 1. HERE - making the (max) length of the supply chain as long as in 1976
foreach v of varlist mu_0 mu_1 mu_2 mu_3 mu_4 mu_5 mu_6 mu_7 mu_8 mu_9{
	replace `v' = 1.32 if `v' !=.
	}

* 2. HERE - making the (max) length of the supply chain as long as in 1976
foreach v of varlist endPoint_4 endPoint_5 endPoint_6 endPoint_7 endPoint_8 endPoint_9 {
	replace `v' =.
	}
...
forval step = 0/8{
	...
	* 3. HERE - making sure the number of supplier is the same as in 1976
	* Making all firms have only 1 supplier (median in 1976)
	*set seed 1
	*gen r=runiform()
	*sort endPoint_`j' r
	*by endPoint_`j': gen n=_n
	*drop if n!=1 & endPoint_`j'!=.
	*drop n r
	...
	*4. HERE - Setting the markup to that of the lowest markup supplier
	bys endPoint_`j': egen min_mu = min(mu_`i')
	replace mu_`i' = min_mu if mu_`i'!=.
	drop min_mu
...
twoway (scatter ratio_all_ma year, connect(direct)) ///
		(scatter ratio_last year, connect(direct))

save "$data/009_calibration/${doNum}_welfare_constantNSuppliers.dta", replace
